  ...
  function processNode(node, depth = 0) {
    ...
    // Handle text nodes
      ...
      if (text) {
        ...
        const isInlineParent = ['SPAN', 'STRONG', 'EM', 'B', 'I', 'MARK', 'A'].includes(
          parentTagName
        );
...
function setupQuillCopyHandler(quillInstance) {
  ...
  editorElement.addEventListener('copy', e => {
    ...
    try {
      ...
      const hasBulletList =
        /data-list="bullet"/i.test(selectedHtml) || processDiv.querySelector('ul');
      const hasNumberedList =
        /data-list="ordered"/i.test(selectedHtml) || processDiv.querySelector('ol');
    ...
    if (navigator.clipboard.read) {
      try {
        ...
        for (const item of items) {
          ...
          if (
            item.types.some(t => t.startsWith('image/') && !t.includes('docs')) &&
            item.types.length === 1
          ) {
