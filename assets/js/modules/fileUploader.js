...
async function processImageWithOCR(file) {
    try {
        if (file.type.toLowerCase().includes('image')) {
            if (isMobileDeviceForImageForImage() ||
...
function OCRImproveCall(text) {
    ...
    formData.append('translateTo', deps.getLanguageName ? deps.getLanguageName(deps.getCurrentLanguage ? deps.getCurrentLanguage() : 'da') : 'Danish');
...
function initializeFileUpload(dependencies) {
    ...
    imageUpload.setAttribute('accept', 'image/*,.pdf,.docx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document');
